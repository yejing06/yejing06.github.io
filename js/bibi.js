...
let items = []
let page = 1
let Url = 'https://kkapi.naokuoteng.cn/api/ispeak?author=6398258744c7091a5c49302c&page=' // 修改api，记得带参数page
...
function getNew() {
    ...
    fetch(Url + page).then(res => res.json()).then((res) => {
        ...
        nowNum += items.length
        if (page == 1) {
            document.querySelector('.bb-info').innerHTML = '<i class="far fa-comment-alt"></i> 我的说说(' + total + ')'
        }
        page += 1
    }).then(() => {
        bb();
